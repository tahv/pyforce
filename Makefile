...
$(venv): pyproject.toml
	...
	python -m venv $(venv)
	$(python) -m pip install build --editable .[dev]
else
$(venv): pyproject.toml
	@$(MAKE) --no-print-directory uninstall
	python -m venv $(venv)
...
tests: $(venv)
	-docker stop pyforce-test && docker rm pyforce-test
	docker build --quiet --platform linux/amd64 -t pyforce .
	docker run --name pyforce-test pyforce "python -m coverage run -m pytest"
	docker cp pyforce-test:/app/.coverage .coverage
	$(python) -m coverage report --show-missing --skip-covered --skip-empty

.PHONY: interactive  ## Run an interactive docker container
interactive:
	docker build --platform linux/amd64 -t pyforce .
	docker run --rm -it pyforce -c "mkdir -p /depot && p4d -r /depot -r localhost:1666 --daemonsafe -L /app/p4dlogs && /bin/sh"
